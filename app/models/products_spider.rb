class ProductsSpider < Kimurai::Base
    ...
    def parse(response, url:, data: {})
      response.xpath("//div[@class='ui-search-result__wrapper']").each do |product|
        ...
        item[:price] = product.css('span.price-tag-fraction')&.text&.squish&.delete('^0-9').to_d
        item[:link] = product.css('a')[0].attributes["href"].value
        shipping = product.css('p.ui-search-item__shipping')&.text&.squish
        ...
        if shipping.present?
          ...
          item[:shipping] = product.css('span.ui-search-item__promise__text')&.text&.squish
